  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setCategories(
          (Array.isArray(catRes) ? catRes : catRes?.data || []).filter(
            (c) => c.isActive !== false,
          ),
        );
        setBrands(
          (Array.isArray(brandRes) ? brandRes : brandRes?.data || []).filter(
            (b) => b.isActive !== false,
          ),
        );
      } catch (err) {
  ...
  return (
    <>
          ...
          <div className="max-w-[1400px] mx-auto px-8 h-6 flex items-center justify-between">
            ...
            <div className="relative flex-1 overflow-hidden h-full flex items-center">
              <div className="animate-marquee whitespace-nowrap flex">
                <span className={`text-[10px] uppercase ${announceText} pr-20`}>
                  CHRONOS: Nơi thời gian trở thành di sản.
                </span>
                {/* Lặp lại slogan để tạo hiệu ứng nối đuôi vô tận */}
                <span className={`text-[10px] uppercase ${announceText} pr-20`}>
                  CHRONOS: Nơi thời gian trở thành di sản.
                </span>
              </div>
            ...
            <div className="flex items-center gap-6 ml-4">
              {isAuthenticated ? (
                ...
                <div
                  className={`flex items-center gap-4 text-[10px] uppercase ${announceText}`}
                >
                  <Link
                    to="/login"
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <nav className="hidden md:flex items-center gap-10 flex-1">
              ...
              >
                ...
                >
                  Bộ Sưu Tập
                  <ChevronDown
                    className={`w-3.5 h-3.5 transition-transform duration-300 ${activeDropdown === "collections" ? "rotate-180 text-amber-500" : ""}`}
                  />
                </Link>
                ...
                {activeDropdown === "collections" && (
                  ...
                  >
                    <div className="py-2">
                      ...
                      <div
                        className={`mt-2 pt-2 border-t px-4 pb-2 ${isDark ? "border-white/5" : "border-zinc-100"}`}
                      >
                        <Link
                          to="/products"
                          className="text-[10px] text-amber-500 hover:text-amber-400 uppercase font-bold transition-colors"
                        >
                          Xem tất cả →
                        </Link>
                      </div>
              ...
              <Link
                to="/about"
                className={`text-[12px] font-semibold uppercase transition-colors duration-300 ${navLink}`}
              >
                Về Chúng Tôi
              </Link>
            </nav>
            ...
            <div className="absolute left-1/2 -translate-x-1/2">
              <Link to="/" className="flex flex-col items-center group">
                <span
                  className={`text-xl md:text-2xl font-bold  uppercase transition-colors duration-500 ${logoText}`}
            ...
            <div className="flex items-center gap-4 md:gap-6 flex-1 justify-end">
              ...
              <div className="relative">
                ...
                {searchOpen && (
                  <div
                    className={`absolute top-full right-0 mt-4 w-72 border backdrop-blur-2xl animate-fadeInDown ${dropBg}`}
                  >
                    <form onSubmit={handleSearch} className="flex items-center">
                      ...
                      <button
                        type="submit"
                        className="px-4 py-3.5 text-amber-500 hover:text-amber-400 transition-colors"
                      >
              ...
              >
                ...
                {activeDropdown === "user" && (
                  <div
                    className={`absolute top-full right-0 mt-4 w-52 border backdrop-blur-2xl animate-fadeInDown ${dropBg}`}
                  >
                    ...
                    {isAuthenticated ? (
                      <>
                        <div
                          className={`px-4 py-3.5 border-b ${isDark ? "border-white/8" : "border-zinc-100"}`}
                        >
                          <p className="text-[12px] text-amber-500 uppercase  mb-1">
                            Thành Viên
                          </p>
                          <p
                            className={`text-sm font-medium truncate ${isDark ? "text-white" : "text-zinc-900"}`}
                          >
                            {user?.full_name || user?.username}
                          </p>
                        </div>
                        <div className="py-1">
                          <Link
                            to="/profile"
                            className={`flex items-center gap-3 px-4 py-2.5 text-[11px]  uppercase transition-all ${dropItem}`}
                          >
                          ...
                          {user?.role === "admin" && (
                            <Link
                              to="/admin"
                              className="flex items-center gap-3 px-4 py-2.5 text-[11px]  uppercase text-amber-500 hover:text-amber-400 hover:bg-amber-500/5 transition-all"
                            >
                      ...
                      <>
                        ...
                        <div className="py-1">
                          ...
                          <Link
                            ...
                            className={`flex items-center gap-3 px-4 py-2.5 text-[11px]  uppercase transition-all ${dropItem}`}
                          >
                            <User className="w-3.5 h-3.5" /> Đăng Ký
                          </Link>
                        </div>
                      </>
                    )}
                    ...
                    <div
                      className={`border-t py-1 ${isDark ? "border-white/8" : "border-zinc-100"}`}
                    >
                      <button
                        onClick={toggleTheme}
                        className={`w-full flex items-center gap-3 px-4 py-2.5 text-[11px] uppercase transition-all ${dropItem}`}
                      >
                        {isDark ? (
                    ...
                    {isAuthenticated && (
                      ...
                      >
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-4 py-2.5 text-[11px]  uppercase text-red-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/5 transition-all"
                        >
      ...
      >
        ...
        ></div>
        <div
          className={`absolute top-0 left-0 bottom-0 w-[80vw] max-w-sm flex flex-col transition-transform duration-500 border-r ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} ${isDark ? "bg-zinc-950 border-white/10" : "bg-white border-zinc-200"}`}
        >
          <div
            className={`flex items-center justify-between px-6 py-5 border-b ${isDark ? "border-white/8" : "border-zinc-100"}`}
          >
            <Link
              to="/"
              className={`text-lg font-bold  uppercase ${isDark ? "text-white" : "text-zinc-900"}`}
              style={{ fontFamily: "Georgia, serif" }}
              onClick={() => setMobileMenuOpen(false)}
            >
              CHRONOS
            ...
            <button
              onClick={() => setMobileMenuOpen(false)}
              className={`p-1 ${isDark ? "text-zinc-400 hover:text-white" : "text-zinc-400 hover:text-zinc-900"}`}
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <nav className="flex-1 overflow-y-auto py-4">
            <div className="px-4 pb-2">
              <p
                className={`text-[12px]  uppercase px-2 mb-3 ${isDark ? "text-amber-500/70" : "text-amber-600/60"}`}
              >
                Menu
              </p>
              {[
            ...
            {/* Theme toggle mobile */}
            <div
              className={`mx-4 mt-2 px-2 py-3 border rounded flex items-center justify-between ${isDark ? "border-white/8" : "border-zinc-100"}`}
            >
              <span
              ...
              <button
                onClick={toggleTheme}
                className={`flex items-center gap-2 text-[11px] font-bold uppercase  transition-colors ${isDark ? "text-amber-400" : "text-amber-600"}`}
              >
                {isDark ? (
                  <>
          ...
          <div
            className={`border-t px-6 py-5 ${isDark ? "border-white/8" : "border-zinc-100"}`}
          >
            {isAuthenticated ? (
              <div className="space-y-2">
                <p
                  className={`text-xs mb-3 ${isDark ? "text-zinc-500" : "text-zinc-400"}`}
                >
                  Xin chào,{" "}
                ...
                <Link
                  to="/profile"
                  className={`block py-1 text-sm transition-colors ${isDark ? "text-zinc-300 hover:text-white" : "text-zinc-600 hover:text-zinc-900"}`}
                  onClick={() => setMobileMenuOpen(false)}
                >
                  Hồ Sơ
                </Link>
                ...
                <button
                  ...
                  className="block py-1 text-sm text-red-400 hover:text-red-500 transition-colors"
                >
                  Đăng Xuất
                </button>
              </div>
    ...
    /* Thêm đoạn này để slogan có thể chạy */
    @keyframes marquee {
        0% { transform: translateX(0); }
        100% { transform: translateX(-50%); } /* Chạy đến một nửa vì ta nhân đôi nội dung */
    }
    .animate-marquee {
        display: flex;
