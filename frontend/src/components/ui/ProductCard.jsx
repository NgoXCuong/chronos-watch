...
const ProductCard = ({ product }) => {
  ...
  return (
    <div
      className={`group relative border overflow-hidden flex flex-col transition-all duration-500 h-full ${
        isDark
    ...
    >
      {/* Discount badge */}
      {discount && (
        <div className="absolute top-3 left-3 z-10 bg-amber-600 rounded-full text-white text-[12px] font-bold px-2 py-1 uppercase">
          -{discount}%
        </div>
      )}
      ...
      <Link
        to={`/products/${slug}`}
        className={`block relative overflow-hidden ${isDark ? "bg-zinc-800" : "bg-slate-50"}`}
        style={{ aspectRatio: "1" }}
      >
        <img
          src={mainImage}
      ...
      <div className="flex flex-col flex-1 p-4">
        <p className="text-[12px] text-amber-500 uppercase mb-1.5 font-bold">
          {product?.brand?.name || "Chronos"}
        </p>
        <Link to={`/products/${slug}`}>
          <h3
            className={`text-sm font-medium line-clamp-2 mb-3 leading-snug transition-colors ${
              isDark
        ...
        <div className="mt-auto">
          ...
          <Button
            ...
            className={`w-full text-[10px] uppercase font-bold transition-all duration-300 py-5 ${
              isDark
