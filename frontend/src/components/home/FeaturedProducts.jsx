...
const ProductCard = ({ product, isDark }) => {
    ...
    return (
        <div className={`group relative border overflow-hidden flex flex-col transition-all duration-500 ${isDark
            ...
            {/* Discount badge */}
            {discount && (
                <div className="absolute top-3 left-3 z-10 bg-amber-600 text-white text-[9px] font-bold  px-2 py-1 uppercase">
                    -{discount}%
                </div>
            )}

            {/* Wishlist */}
            <button className={`absolute top-3 right-3 z-10 w-8 h-8 flex items-center justify-center border backdrop-blur-sm transition-all duration-300 opacity-0 group-hover:opacity-100 ${isDark
            ...
            {/* Image */}
            <Link to={`/products/${slug}`} className={`block relative overflow-hidden ${isDark ? 'bg-zinc-800' : 'bg-zinc-50'}`} style={{ aspectRatio: '1' }}>
            ...
            <div className="flex flex-col flex-1 p-4">
                <p className="text-[9px]  text-amber-500 uppercase mb-1.5">
                    {product?.brand?.name || 'Chronos'}
                </p>
                <Link to={`/products/${slug}`}>
                    <h3 className={`text-sm font-medium line-clamp-2 mb-2 leading-snug transition-colors ${isDark
                ...
                <div className="flex items-center gap-1 mb-3">
                    ...
                    <span className={`text-[9px] ml-1 ${isDark ? 'text-zinc-600' : 'text-zinc-400'}`}>(24)</span>
                </div>
                ...
                <div className="mt-auto flex items-center justify-between">
                    ...
                    <Link to={`/products/${slug}`}
                        className={`flex items-center gap-1.5 px-3 py-2 text-[9px] uppercase font-bold border transition-all duration-300 ${isDark
...
const FeaturedProducts = () => {
    ...
    useEffect(() => {
        productApi.getAll({ limit: 8, sort: 'createdAt', order: 'desc' })
            .then(data => {
                // Sequelize findAndCountAll returns {count, rows}
                const items = data?.rows || (Array.isArray(data) ? data : (data?.data || data?.products || []));
                setProducts(items);
            })
            .catch(() => setProducts([]))
        ...
        return (
            <section className="flex items-center justify-center py-20">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className={`text-[10px] uppercase ${isDark ? 'text-zinc-600' : 'text-zinc-400'}`}>Đang tải...</p>
                </div>
            </section>
        );
    }
        ...
        <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                {displayProducts.slice(0, 8).map(product => (
                    <ProductCard key={product._id || product.id} product={product} isDark={isDark} />
                ))}
            </div>
            ...
            <div className="mt-8 text-center sm:hidden">
                <Link to="/products"
                    className={`inline-flex items-center gap-2 px-8 py-3 border text-[11px]  uppercase transition-all ${isDark
