...
const StatCard = ({
  ...
  return (
    <div
      className={cn(
        "p-6 rounded-xl border border-slate-100 shadow-sm transition-all duration-500 hover:shadow-xl hover:-translate-y-1 group relative overflow-hidden",
        bg || defaultBg,
      )}
    >
      ...
      <div className="flex items-center justify-between mb-4 relative z-10">
        <div
          className={cn(
            "p-3 rounded-xl border transition-all duration-500 group-hover:rotate-12 shadow-sm shrink-0",
            bg === "bg-white" || !bg
        ...
        >
          {Icon && (
            <Icon className={cn("h-5 w-5", color || "text-amber-500")} />
          )}
        </div>
          ...
          >
            <path
              d={`M ${hist.map((val, i) => {
                ...
                const y = max === 0 ? 40 : 40 - (val / max) * 30;
                return `${x},${y}`;
              }).join(" L ")}`}
              fill="none"
              stroke={chartColor}
