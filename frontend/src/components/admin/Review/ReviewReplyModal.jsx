        ...
        <Dialog open={!!replyingTo} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-md p-0 overflow-hidden border-none shadow-2xl">
                ...
                <DialogFooter className="p-8 pt-0 flex gap-3">
                    <Button variant="ghost" onClick={onClose} className="rounded-2xl h-12 flex-1 font-bold text-slate-400 uppercase text-[10px] hover:bg-slate-50">Hủy bỏ</Button>
                    <Button
                        disabled={submittingReply || !replyText.trim()}
                        onClick={handleReply}
                        className="bg-amber-600 hover:bg-amber-700 text-white rounded-2xl h-12 flex-1 font-bold uppercase text-[10px] shadow-lg shadow-amber-600/20 gap-2 active:scale-95 transition-all"
                    >
