    ...
    return (
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 border-b border-slate-100">
                        <tr>
                            <th className="px-4 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Tên danh mục</th>
                            ...
                            <th className="px-4 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Cấp</th>
                            <th className="px-4 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider font-bold">Trạng thái</th>
                            <th className="px-4 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Thứ tự</th>
                            ...
                            <tr key={cat.id} className="hover:bg-slate-50/50 transition-colors">
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-1" style={{ paddingLeft: cat.level * 16 }}>
                                        ...
                                        <span className={`${cat.level === 0 ? 'font-bold text-slate-800' : 'font-medium text-slate-600'} text-sm uppercase tracking-tight`}>{cat.name}</span>
                                    </div>
                                </td>
                                ...
                                <td className="px-4 py-3">
                                    <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border shadow-sm uppercase tracking-widest ${cat.level === 0 ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                                        {cat.level === 0 ? 'Gốc' : 'Con'}
                                    </span>
                                </td>
                                <td className="px-4 py-3">
                                    <button onClick={() => onToggle(cat)} className="flex items-center gap-1.5 focus:outline-none">
                                        {cat.is_active !== false
                                            ? <><ToggleRight className="h-4.5 w-4.5 text-emerald-500" /><span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-100 uppercase tracking-widest">Hiện</span></>
                                            : <><ToggleLeft className="h-4.5 w-4.5 text-slate-400" /><span className="text-[10px] font-bold text-slate-400 bg-slate-50 px-2.5 py-0.5 rounded-full border border-slate-100 uppercase tracking-widest">Ẩn</span></>}
                                    </button>
                                </td>
            ...
            <div className="px-4 py-2.5 border-t border-slate-50 bg-slate-50/30">
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Tổng: {categories.length} danh mục</p>
            </div>
        </div>
    );
};
