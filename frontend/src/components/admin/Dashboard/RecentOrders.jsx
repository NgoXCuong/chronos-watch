    ...
    return (
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between mb-6">
                ...
                <Button variant="ghost" onClick={() => navigate('/admin/orders')} className="text-xs font-bold text-amber-600 gap-1 hover:bg-amber-50">
            ...
            <div className="space-y-4">
                {orders && orders.length > 0 ? (
                    orders.map((order, idx) => (
                        <div key={idx} className="flex items-center justify-between p-4 bg-slate-50/50 rounded-2xl border border-transparent hover:border-amber-200 transition-all cursor-pointer group" onClick={() => navigate(`/admin/orders/${order.id}`)}>
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-slate-800">{order.user?.username || 'Khách vãng lai'}</p>
                            ...
                            <div className="text-right">
                                ...
                                <span className={`text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${order.status === 'delivered' ? 'bg-emerald-50 text-emerald-600' :
                                        order.status === 'cancelled' ? 'bg-rose-50 text-rose-600' : 'bg-blue-50 text-blue-600'
                                    }`}>
                                    {order.status === 'pending' ? 'Chờ duyệt' :
                                        order.status === 'processing' ? 'Đang giao' :
                                            order.status === 'delivered' ? 'Xong' : order.status}
                                </span>
                            </div>
                        </div>
                    ))
                ) : (
                    <div className="py-10 text-center text-slate-400 font-medium italic text-xs">
