...
const COLORS = ['#F59E0B', '#10B981', '#3B82F6', '#8B5CF6', '#F43F5E', '#94A3B8'];

const OrderStatusPie = ({ data, totalOrders }) => {
    return (
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
            <div className="mb-6">
                <h3 className="text-lg font-bold text-slate-900">Trạng thái đơn hàng</h3>
                <p className="text-xs text-slate-400 font-medium mt-1">Cơ cấu vận hành hiện tại</p>
            </div>
            <div className="h-64 relative">
                {data && data.length > 0 ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-50/50 rounded-full border-2 border-dashed border-slate-100 mx-auto w-48 h-48">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Trống</p>
                    </div>
                )}
            </div>
            <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2">
                {data && data.length > 0 ? data.map((entry, index) => (
                    <div key={index} className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-slate-900 ml-auto">{entry.value}</span>
                    </div>
                )) : (
                    <div className="col-span-2 text-center py-2 text-[10px] text-slate-400 font-medium italic">
