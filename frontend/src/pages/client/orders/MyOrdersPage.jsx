...
const STATUS_MAP = {
  ...
  processing: {
    label: "Đang xử lý",
    color: "text-zinc-600 dark:text-zinc-600",
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-6">
          ...
          <h1 className="text-4xl md:text-6xl text-zinc-900 dark:text-white leading-tight mb-2">
            Hồ Sơ{" "}
            <span className="italic font-light text-zinc-700 dark:text-zinc-600">
              Sở Hữu
            </span>
          </h1>
          <div className="h-px w-32 bg-linear-to-b from-amber-500 to-transparent mx-auto mt-2 mb-2"></div>
          <p className="text-zinc-700 dark:text-zinc-600 font-light max-w-2xl mx-auto text-sm md:text-base leading-relaxed">
            Lưu giữ dấu ấn của những tuyệt tác vi cơ học mà bạn đã lựa chọn để
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-2 border-b border-zinc-200 dark:border-zinc-800">
          ...
          <div className="flex items-center gap-8 text-center">
            <div>
              ...
              <p className="text-[10px]  text-zinc-700 uppercase mt-1">
                Tuyệt tác
              </p>
            </div>
          </div>
        ...
        {filteredOrders.length === 0 ? (
          <div className="text-center py-16 bg-white dark:bg-zinc-900 rounded-sm border border-zinc-200 dark:border-zinc-800">
            ...
            />
            <h3 className="text-xl font-serif text-zinc-900 dark:text-white mb-3">
              Hồ sơ trống
            </h3>
                ...
                >
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                    ...
                    <div className="w-full md:w-auto">
                      {order.details?.length > 1 && (
                        <div className="space-y-4">
                          <p className="text-[12px] text-zinc-600 uppercase font-black">
                            Các sản phẩm đi kèm
                          </p>
                          <div className="flex items-center gap-3 overflow-x-auto pb-2 scrollbar-none">
                            {order.details.slice(1, 5).map((item, idx) => (
                              <div
                                key={idx}
                                className="h-14 w-14 bg-zinc-50 dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-800 p-1.5 shrink-0 rounded-sm opacity-60 hover:opacity-100 transition-opacity"
                              >
                                <img
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {order.status === "delivered" && firstProduct && (
                          <button
                            onClick={() =>
                              navigate(
                                `/products/${firstProduct.slug || firstProduct.id}#reviews`,
                              )
                            }
                            className="h-12 px-5 flex items-center gap-2 bg-amber-600 text-white text-[10px] font-bold uppercase hover:bg-amber-700 transition-all rounded-sm"
                          >
                            Đánh giá
                          </button>
                        )}
