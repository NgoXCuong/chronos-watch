  ...
  if (orders.length === 0) {
    return (
      <div className="text-center py-20 bg-zinc-100/50 dark:bg-zinc-900/30 rounded-3xl border border-zinc-200 dark:border-zinc-800/50 border-dashed">
        ...
        <h3 className="text-xl font-serif text-zinc-900 dark:text-white mb-2 font-medium">
          Chưa có đơn hàng nào
        </h3>
        <p className="text-zinc-700 font-light max-w-xs mx-auto">
          Hãy khám phá bộ sưu tập đồng hồ tinh tế của chúng tôi và đặt đơn hàng
          đầu tiên.
        </p>
        <button
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-4 border-b border-zinc-200 dark:border-zinc-800/60">
        <div className="space-y-2">
          ...
          <h2 className="text-2xl md:text-3xl font-serif font-light text-zinc-900 dark:text-white ">
            Lịch Sử{" "}
            <span className="italic text-zinc-600 dark:text-zinc-700">
              Sở Hữu
            </span>
          </h2>
        </div>
            ...
            >
              ...
              <div className="flex-1 space-y-3">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                      <span className="text-[12px] font-black text-amber-600 uppercase ">
                        OWNERSHIP ID
                      </span>
                      <span className="text-[12px] text-zinc-600 dark:text-zinc-700 font-light">
                        #{order.id}
                      </span>
                    </div>
                    ...
                    {order.details?.length > 1 && (
                      <p className="text-[12px] text-zinc-600 italic">
                        +{order.details.length - 1} vật phẩm khác
                      </p>
                    )}
                  </div>

                  <span
                    className={`inline-flex items-center px-2 py-1 rounded-sm text-[10px] font-black uppercase  border ${getStatusColor(order.status)}`}
                  >
                ...
                <div className="flex flex-wrap items-center gap-6 pt-2">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[11px] text-zinc-700 font-bold uppercase ">
              ...
              <div className="flex items-end gap-3">
                ...
                {order.status === "delivered" && firstItem && (
                  <a
                    href={`/products/${firstItem.slug || firstItem.id}#reviews`}
                    className="inline-flex items-center gap-2 text-[12px] font-black uppercase bg-amber-600 text-white px-3 py-2 rounded-md hover:bg-amber-700 transition-all"
                  >
                    Đánh giá
                  </a>
                )}
