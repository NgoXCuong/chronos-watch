...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    if (
      ["profile", "orders", "wishlist", "security", "addresses"].includes(hash)
    ) {
      setActiveTab(hash);
    }
  }, [location]);
      ...
      <div className="max-w-6xl mx-auto space-y-8 relative z-10">
        ...
        <div className="text-center space-y-3 mb-10">
          ...
          <h1 className="text-3xl md:text-5xl text-zinc-900 dark:text-white leading-tight font-light ">
            Hồ Sơ{" "}
            <span className="italic font-serif text-zinc-600 dark:text-zinc-700">
              Cá Nhân
            </span>
          </h1>
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div className="flex-1 bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800/60 rounded-sm p-6 md:p-8 shadow-sm relative overflow-hidden group/content">
            ...
            <div className="relative z-10 min-h-75">{renderTabContent()}</div>
          </div>
        </div>
      </div>
    </div>
...
const WishlistTab = ({ wishlist, toggleWishlist }) => {
  if (wishlist.length === 0) {
    return (
      <div className="text-center py-20 bg-zinc-50/50 dark:bg-zinc-950/30 rounded-sm border border-zinc-200 dark:border-zinc-800 border-dashed">
        ...
        <p className="text-xs text-zinc-600 font-light max-w-xs mx-auto italic">
          Sản phẩm bạn yêu thích sẽ được lưu giữ tại đây.
        </p>
        <Link to="/products">
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <h2 className="text-2xl font-serif font-light text-zinc-900 dark:text-white mb-6 ">
        Sản phẩm <span className="italic text-zinc-600">Yêu Thích</span>
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
        {wishlist.map((item) => (
          <div
            key={item.id}
            className="group relative bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-sm p-4 hover:border-amber-500/30 transition-all duration-500 shadow-sm"
          >
            <button
            ...
            <Link
              ...
              className="block aspect-square rounded-sm overflow-hidden bg-zinc-50 dark:bg-zinc-950 mb-4"
            >
              <img
                src={item.image_url}
                alt={item.name}
