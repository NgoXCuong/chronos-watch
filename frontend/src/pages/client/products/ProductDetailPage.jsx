...
const ProductDetailPage = () => {
      ...
      .then((data) => {
        ...
        productApi
          ...
          .then((relData) => {
            ...
            setRelatedProducts(
              items.filter((p) => (p.id || p._id) !== data.id),
            );
          });
      })
  ...
  if (loading) {
    return (
      <div
        className={`min-h-screen flex items-center justify-center ${isDark ? "bg-[#080808]" : "bg-white"}`}
      >
        <div className="relative">
          ...
          <div className="mt-4 text-[10px] uppercase text-amber-500 font-bold text-center">
            Chronos
          </div>
        </div>
      </div>
  ...
  if (!product) {
    return (
      <div
        className={`min-h-screen flex flex-col items-center justify-center p-6 text-center ${isDark ? "bg-[#080808]" : "bg-white"}`}
      >
        <h2
          className={`text-3xl font-bold mb-4 ${isDark ? "text-white" : "text-zinc-900"}`}
          style={{ fontFamily: "Cinzel, serif" }}
        >
          Tuyệt phẩm không tìm thấy
        </h2>
        <p className={`mb-8 ${isDark ? "text-zinc-700" : "text-zinc-600"}`}>
          Hiện tại chúng tôi không tìm thấy sản phẩm bạn yêu cầu.
        </p>
        <Link to="/products">
  ...
  const getStockStatus = () => {
    ...
    if (product.stock <= 5)
      return {
        label: "Chỉ còn " + product.stock + " chiếc",
  ...
  return (
    <div
      className={`min-h-screen transition-colors duration-500 ${isDark ? "bg-[#080808]" : "bg-white"}`}
    >
      <ProductBreadcrumbs product={product} isDark={isDark} />

      <div className="max-w-340 mx-auto px-4 md:px-8 py-4 md:py-6">
