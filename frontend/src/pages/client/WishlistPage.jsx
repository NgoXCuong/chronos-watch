...
const WishlistPage = () => {
  ...
  if (wishlistCount === 0) {
    return (
      <div
        className={`min-h-[70vh] flex flex-col items-center justify-center p-6 text-center ${isDark ? "bg-[#080808]" : "bg-white"}`}
      >
        ...
        <h2
          className={`text-2xl font-bold mb-4 ${isDark ? "text-white" : "text-zinc-900"}`}
          style={{ fontFamily: "Georgia, serif" }}
        >
          Danh sách yêu thích trống
        </h2>
        <p
          className={`max-w-xs mx-auto mb-10 text-sm leading-relaxed ${isDark ? "text-zinc-700" : "text-zinc-600"}`}
        >
          Lưu giữ những tuyệt phẩm bạn trân quý nhất tại đây để không bỏ lỡ
          khoảnh khắc sở hữu chúng.
        </p>
        <Link to="/products">
          <Button
            variant="outline"
            className="px-10 h-14 rounded-none uppercase text-[14px] font-bold  transition-all"
          >
            Khám phá bộ sưu tập
          </Button>
        </Link>
  ...
  return (
    <div
      className={`min-h-screen pb-20 ${isDark ? "bg-[#080808]" : "bg-white"}`}
    >
      <main className="max-w-350 mx-auto px-6 pt-4 md:pt-6">
        <header className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6 border-b dark:border-white/5 border-zinc-100 pb-8">
          <div>
            <div className="flex items-center gap-3 mb-4 animate-in fade-in duration-700">
              ...
              <span className="text-[10px] uppercase font-black text-amber-500">
                Bộ Sưu Tập Riêng Tư
              </span>
            </div>
            <h1
              className={`text-3xl md:text-4xl font-bold uppercase  ${isDark ? "text-white" : "text-zinc-900"}`}
            >
              Yêu Thích{" "}
              <span className="text-zinc-700 ml-2 font-light">
                ({wishlistCount})
              </span>
            </h1>
          </div>
          <Link
            to="/products"
            className={`flex items-center gap-2 text-[10px] uppercase font-bold  transition-colors ${isDark ? "text-zinc-700 hover:text-white" : "text-zinc-600 hover:text-zinc-900"}`}
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {wishlist.map((product) => (
            <div
              key={product.id}
              className={`group relative flex flex-col border p-5 transition-all duration-700
            ...
            >
              ...
              <Link
                ...
                className="block relative overflow-hidden bg-white mb-6"
                style={{ aspectRatio: "1" }}
              >
                <img
                  src={
              ...
              <div className="flex flex-col flex-1">
                ...
                <Link to={`/products/${product.slug}`}>
                  <h3
                    className={`text-sm font-bold mb-4 line-clamp-2 transition-colors ${isDark ? "text-white group-hover:text-amber-500" : "text-zinc-900 group-hover:text-amber-600"}`}
                  >
                    {product.name}
                  </h3>
                </Link>
                ...
                <div className="mt-auto pt-4 border-t dark:border-white/5 border-zinc-100 flex items-center justify-between gap-4">
                  <div>
                    <p
                      className={`text-[12px] font-black ${isDark ? "text-white" : "text-zinc-900"}`}
                    >
